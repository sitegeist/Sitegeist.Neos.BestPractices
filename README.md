...
1. The Neos Best Practices as described in https://www.neos.io/blog/neos-best-practices-1-0.html 
   MUST be respected.
2. All projects MUST contain a `Makefile` and implement the cli-commands `make install`, `make build` 
   and `make run`. It is RECOMMENDED to also implement `make watch`, `make lint`, `make test` and make `deploy-*`.
